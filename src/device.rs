use crate::{
    target_info::Dep, wrap_err, BaudRate, Context, DepMode, Error, Mode, Modulation,
    ModulationType, Property, Result, Target, Timeout,
};
use nfc1_sys::{
    nfc_abort_command, nfc_close, nfc_device, nfc_device_get_connstring,
    nfc_device_get_information_about, nfc_device_get_last_error, nfc_device_get_name,
    nfc_device_get_supported_baud_rate, nfc_device_get_supported_baud_rate_target_mode,
    nfc_device_get_supported_modulation, nfc_device_set_property_bool, nfc_device_set_property_int,
    nfc_free, nfc_idle, nfc_initiator_deselect_target, nfc_initiator_init,
    nfc_initiator_init_secure_element, nfc_initiator_list_passive_targets,
    ...
    nfc_initiator_transceive_bytes, nfc_initiator_transceive_bytes_timed, nfc_open, nfc_strerror,
    nfc_target_init, nfc_target_receive_bits, nfc_target_receive_bytes, nfc_target_send_bits,
    nfc_target_send_bytes, pn53x_read_register, pn53x_transceive, pn53x_write_register, size_t,
};
...
impl<'a> Device<'a> {
    ...
    pub fn initiator_select_passive_target_with_init_data(
        &mut self,
    ...
    pub fn initiator_list_passive_targets(
        ...
        let mut targets: Vec<nfc1_sys::nfc_target> =
            vec![(&Target::new_iso14443a()).into(); max_len];
        wrap_err(unsafe {
            nfc_initiator_list_passive_targets(
                self.ptr,
                modulation.into(),
                targets.as_mut_ptr(),
                targets.len() as size_t,
            )
        })?;
        targets
            .into_iter()
    ...
    pub fn initiator_poll_target(
        ...
        let mut target: nfc1_sys::nfc_target = (&Target::new_iso14443a()).into();
        let modulations: Vec<nfc1_sys::nfc_modulation> = modulations
            .iter()
            .map(|modulation| modulation.into())
            .collect();
    ...
    pub fn initiator_poll_dep_target(
        &mut self,
        dep_mode: DepMode,
        baud_rate: BaudRate,
        initiator: &Dep,
    ...
    pub fn initiator_transceive_bytes(
        &mut self,
        tx: &[u8],
        rx_len: usize,
        timeout: Timeout,
    ...
    pub fn initiator_transceive_bits_with_parity(
        &mut self,
        tx: &[u8],
        tx_bits: usize,
        parity_tx: &[u8],
    ...
    pub fn initiator_transceive_bits_with_parity_timed(
        &mut self,
        tx: &[u8],
        tx_bits: usize,
        parity_tx: &[u8],
    ...
    pub fn target_send_bytes(&mut self, tx: &[u8], timeout: Timeout) -> Result<()> {
        wrap_err(unsafe {
            nfc_target_send_bytes(self.ptr, tx.as_ptr(), tx.len() as size_t, timeout.into())
        })
    }
    ...
    pub fn target_send_bits(&mut self, tx: &[u8], tx_bits: usize) -> Result<()> {
        ...
        wrap_err(unsafe {
            nfc_target_send_bits(self.ptr, tx.as_ptr(), tx_bits as size_t, ptr::null_mut())
        })
    }

    pub fn target_send_bits_with_parity(
        &mut self,
        tx: &[u8],
        tx_bits: usize,
        parity_tx: &[u8],
        ...
        wrap_err(unsafe {
            nfc_target_send_bits(self.ptr, tx.as_ptr(), tx_bits as size_t, parity_tx.as_ptr())
        })
    }
    ...
    pub fn name(&mut self) -> &'static str {
        ...
        unsafe { CStr::from_ptr(nfc_device_get_name(self.ptr)) }
            .to_str()
            .unwrap()
    }
    ...
    pub fn connstring(&mut self) -> &'static str {
        ...
        // which is guaranteed to be initialized
        unsafe { CStr::from_ptr(nfc_device_get_connstring(self.ptr)) }
            .to_str()
            .unwrap()
    }
    ...
    pub fn get_supported_modulation(&mut self, mode: Mode) -> Result<Vec<ModulationType>> {
        ...
        wrap_err(unsafe {
            nfc_device_get_supported_modulation(self.ptr, mode.into(), supported_mt.as_mut_ptr())
        })?;
        unsafe {
    ...
    pub fn get_supported_baud_rate(
        &mut self,
        mode: Mode,
        ...
        match mode {
            Mode::Initiator => wrap_err(unsafe {
                nfc_device_get_supported_baud_rate(
                    self.ptr,
                    modulation_type.into(),
                    supported_br.as_mut_ptr(),
                )
            })?,
            Mode::Target => wrap_err(unsafe {
    ...
    pub fn get_information_about(&mut self) -> Result<String> {
        ...
        let strinfo = unsafe { CStr::from_ptr(strinfo_ptr) }
            .to_string_lossy()
            .into_owned();
        unsafe {
            nfc_free(strinfo_ptr as *mut c_void);
        }
        Ok(strinfo)
    }
}
